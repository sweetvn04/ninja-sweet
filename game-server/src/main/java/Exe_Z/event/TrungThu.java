...
import static Exe_Z.event.TrungThu.EXPIRE_30_DAY;
import static Exe_Z.event.TrungThu.EXPIRE_7_DAY;
import static Exe_Z.event.TrungThu.HOA_PHUC_SINH;
import static Exe_Z.event.TrungThu.TOP_BANH_TRUNG_THU;
import static Exe_Z.event.TrungThu.TOP_LONG_DEN;
...
public class TrungThu extends Event {
    ...
    public TrungThu() {
        ...
        itemsThrownFromMonsters.add(2, ItemName.HAT_SEN);
        itemsThrownFromMonsters.add(3, ItemName.DUONG);
        itemsThrownFromMonsters.add(1, ItemName.DAU_XANH);
        itemsThrownFromMonsters.add(5, ItemName.MUT);
    ...
    public void useItem(Char p, Item item) {
        if (item.id == ItemName.HOP_BANH_THUONG || item.id == ItemName.HOP_BANH_THUONG_HANG) {
            ...
            boolean isDone = useEventItem(p, item.id, rc);
        } else if (item.id == ItemName.LONG_DEN) {
            if (p.getSlotNull() == 0) {
                p.warningBagFull();
                return;
            }
            boolean isDone = useEventItem(p, item.id, itemsRecFromGold2Item);
            if (isDone) {
                p.getEventPoint().addPoint(TrungThu.TOP_LONG_DEN, 1);
            }
