...
import Exe_Z.model.Figurehead;
import Exe_Z.model.Char;
import Exe_Z.effect.Effect;
import Exe_Z.task.GloryTask;
...
import Exe_Z.map.world.Territory;
import Exe_Z.constants.TaskName;
import Exe_Z.convert.Converter;
import Exe_Z.event.Halloween;
import Exe_Z.event.Noel;
...
public class Mob {
    ...
    public static final byte VKBOSS = 17;
    ...
    public void dropItem(Char owner, byte type) {
        try {
            ...
            if (type == ITEM) {
                ...
                if (Event.isGioTo()) {
                    RandomCollection<Integer> rand = RandomItemGioTo.GIO_TO;
                    itemId = rand.next();
                } else {
                ...
                if (zone.map.id == MapName.RUNG_DAO_SAKURA && zone.map.id == MapName.CANH_DONG_FUKI) {
                    if (template.id == MobName.TUONG_GIAC){
                    itemId = RandomItemGioTo.BOSS_TUONG_GIAC.next();
                    }
                } else if (zone.map.id == 134 || zone.map.id == 135 || zone.map.id == 136 || zone.map.id == 137) {
                    itemId = RandomItem.BOSS_LANG_CO.next();
    ...
    public void attack(Char pl, Char owner) {
        if (pl != null && !pl.isDead) {
            ...
            try {
                ...
                int percentadd = Config.getInstance().getPercentDameMob();
                int damemin = this.damageOnPlayer2 * percentadd;
                int damemax = this.damageOnPlayer * percentadd;
                int dameHp = NinjaUtils.nextInt(damemin, damemax);
    ...
    public void dead(Char killer) {
        if (killer != null) {
            ...
            if (this.template.id == MobName.NGUOI_TUYET) {
                ...
                if (this.isBoss) {
                    if (zone.tilemap.isVDMQ() || zone.tilemap.isLangCo()) {
                    ...
                    if (zone.tilemap.isLangCo()) {
                        if (NinjaUtils.nextInt(8) == 0) {
                            ...
                            if (killer.user.tongnap >= 0) {
                                killer.addGold(NinjaUtils.nextInt(1, 1));
                                killer.addYen(NinjaUtils.nextInt(1000, 10000));
                            }
                            if (killer.user.tongnap >= 500000) {
                                killer.addGold(NinjaUtils.nextInt(1, 2));
                                killer.addCoin(NinjaUtils.nextInt(1, 500));
                                killer.addYen(NinjaUtils.nextInt(1000, 10000));
                            }
                            if (killer.user.tongnap >= 2000000) {
                                killer.addCoin(NinjaUtils.nextInt(1, 5000));
                                killer.addGold(NinjaUtils.nextInt(1, 3));
                                killer.addYen(NinjaUtils.nextInt(1000, 10000));
                            }
                            if (killer.user.tongnap >= 5000000) {
                                killer.addGold(NinjaUtils.nextInt(1, 5));
                                killer.addCoin(NinjaUtils.nextInt(1, 10000));
                                killer.addYen(NinjaUtils.nextInt(1000, 10000));
                            }
                        }
                        ...
                        if (type == -1) {
                            if (zone.tilemap.isVDMQ()) {
                                if (killer.isTNP && NinjaUtils.nextInt(250) < 5) {
                                    ...
                                    if (killer.user.tongnap >= 2000000) {
                                        killer.addGold(NinjaUtils.nextInt(1, 2));
                                        killer.addCoin(NinjaUtils.nextInt(1, 1000));
                                        killer.addYen(NinjaUtils.nextInt(1000, 10000));
                                    }
                                    if (killer.user.tongnap >= 5000000) {
                                        killer.addGold(NinjaUtils.nextInt(1, 3));
                                        killer.addCoin(NinjaUtils.nextInt(1, 6000));
                                        killer.addYen(NinjaUtils.nextInt(1000, 10000));
                                    }
                                }
