...
import Exe_Z.model.ConfirmPopup;
...
import Exe_Z.shopcoin.ShopitemTB2;
import Exe_Z.shopcoin.ShopitemThu;
import Exe_Z.shopcoin.ShopitemVT;
...
import Exe_Z.effect.EffectData;
...
public class Service extends AbsService {
    ...
    public void openUIShopItemTB1( String title, String caption) {
        try {
            openUI((byte) 4, title, caption);
            Message ms = new Message(CMD.OPEN_UI_BOX);
            DataOutputStream ds = ms.writer();
            ds.writeInt(0);
            ds.writeByte(ShopitemTB1.item_shop.size());
            for (Item item : ShopitemTB1.item_shop) {
                if (item != null) {
    ...
    public void openUIShopItemTB2( String title, String caption) {
        try {
            openUI((byte) 4, title, caption);
            Message ms = new Message(CMD.OPEN_UI_BOX);
            DataOutputStream ds = ms.writer();
            ds.writeInt(0);
            ds.writeByte(ShopitemTB2.item_shop.size());
            for (Item item : ShopitemTB2.item_shop) {
                if (item != null) {
                    ds.writeShort(item.id);
                    ds.writeBoolean(item.isLock);
                    if (item.template.isTypeBody() || item.template.isTypeNgocKham()) {
     ...
     public void openUIShopItemThu( String title, String caption) {
        try {
            openUI((byte) 4, title, caption);
            Message ms = new Message(CMD.OPEN_UI_BOX);
            DataOutputStream ds = ms.writer();
            ds.writeInt(0);
            ds.writeByte(ShopitemThu.item_shop.size());
            for (Item item : ShopitemThu.item_shop) {
                if (item != null) {
                    ds.writeShort(item.id);
                    ds.writeBoolean(item.isLock);
                    if (item.template.isTypeBody() || item.template.isTypeNgocKham()) {
     ...
     public void openUIShopItemVT( String title, String caption) {
        try {
            openUI((byte) 4, title, caption);
            Message ms = new Message(CMD.OPEN_UI_BOX);
            DataOutputStream ds = ms.writer();
            ds.writeInt(0);
            ds.writeByte(ShopitemVT.item_shop.size());
            for (Item item : ShopitemVT.item_shop) {
                if (item != null) {
                    ds.writeShort(item.id);
                    ds.writeBoolean(item.isLock);
                    if (item.template.isTypeBody() || item.template.isTypeNgocKham()) {
        ...
        try {
            ...
            for (Item item : LinhAn.item_create) {
                if (item != null) {
                    ds.writeShort(item.id);
                    ds.writeBoolean(item.isLock);
                    if (item.template.isTypeBody() || item.template.isTypeBiKip()) {
                        ds.writeByte(item.upgrade);
                    }
                    ds.writeBoolean(item.hasExpire());
                    ds.writeShort(item.getQuantityDisplay());
...
//            ds.writeShort(player.ID_SUSANO);
            ds.writeShort(player.tinhluyen);
            ds.flush();
            sendMessage(ms);
            ms.cleanup();
        } catch (Exception ex) {
...
//            ds.writeShort(player.ID_SUSANO);
    ...
    public void openUIConfirm(int npcId, String title, String[] menu) {
        try {
            Message mss = new Message(CMD.OPEN_UI_CONFIRM);
            DataOutputStream ds = mss.writer();
            ds.writeShort(npcId);
